...
import os, ast, json, csv, hashlib, requests, time
...
OUTDIR = "qiskit_repos"
JSON_OUT = "qiskit_humaneval_dataset.json"
CSV_OUT = "Formatted_ORPO_Dataset.csv"
...
def download_file(item):
    ...
    fname = os.path.join(OUTDIR, repo.replace("/", "_") + "__" + path.replace("/", "_"))
...
def build_datasets(token, max_files=100):
    print(f"[1] Searching GitHub...")
    items = search_qiskit_files(token, max_files)

    print(f"[2] Downloading files...")
    downloaded = []
    ...
    print(f"[3] Extracting functions...")
    dataset = []
    task_id = 0
    ...
    print(f"[4] Deduplicating {len(dataset)} entries...")
    dataset = deduplicate(dataset)
    print(f"[INFO] {len(dataset)} unique tasks remain.")

    with open(JSON_OUT, "w", encoding="utf-8") as f:
        json.dump(dataset, f, indent=4)
    print(f"[SAVED] Q-HumanEval dataset → {JSON_OUT}")

    # Build ORPO dataset CSV
    print(f"[5] Creating ORPO dataset CSV...")
    with open(CSV_OUT, "w", newline="", encoding="utf-8") as f:
        ...
        for d in dataset:
            writer.writerow({
                "prompt": d["prompt"],
                "chosen": d["canonical_solution"],
                "rejected": "# Incomplete or incorrect version\npass"
            })
    print(f"[SAVED] ORPO dataset → {CSV_OUT}")
...
if __name__ == "__main__":
    ...
    build_datasets(args.token, args.max_files)
